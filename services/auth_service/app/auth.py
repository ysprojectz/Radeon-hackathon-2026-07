...
def create_access_token(user: dict, market_override: Optional[str] = None, jti: Optional[str] = None) -> str:
    ...
    market = (market_override or user.get("market_region", "UAE")).upper()
    if market not in VALID_MARKETS:
        market = user.get("market_region", "UAE").upper()
    payload: dict = {
        "sub":    user["email"],
        "role":   user["role"],
        "uid":    user["id"],
...
async def get_current_user(
    ...
    if effective_token:
        ...
        if "market" in payload and payload["market"]:
            user_data["market_region"] = payload["market"]
...
async def login(request: Request, response: Response, form: OAuth2PasswordRequestForm = Depends()):
    ...
    from services.auth_service.app.session_manager import create_session, hash_device
...
async def logout(
    ...
    try:
        ...
        from services.auth_service.app.session_manager import revoke_all_sessions
        jti = None
        email = ""
        if cookie_access:
            try:
                payload = jwt.decode(cookie_access, JWT_SECRET_KEY, algorithms=[JWT_ALGORITHM])
                jti   = payload.get("jti")
...
async def list_active_sessions(current_user: CurrentUser = Depends(get_current_user)):
    ...
    """
    from services.auth_service.app.session_manager import get_user_sessions
...
async def revoke_session(
    ...
    **Response**:
      ...
      - Not found: 404 error
    """
    from services.auth_service.app.session_manager import revoke_session as _revoke_session
